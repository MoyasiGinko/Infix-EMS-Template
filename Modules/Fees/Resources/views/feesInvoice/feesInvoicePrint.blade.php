    ...
    <div class="invoice_wrapper">
                        ...
                        <tr>
                          <td>
                            <div class="addressleft_text">
                              ...
                              <p><span><strong>@lang('student.student_name') </span> <span class="nowrap">:
                                  {{$invoiceInfo->studentInfo->full_name}}</span> </strong></p>
                              <p><span>@if(shiftEnable()) @lang('admin.class_Sec_shift') @else @lang('admin.class_Sec')
                                  @endif</span> <span>: {{@$invoiceInfo->recordDetail->class->class_name}}
                                  ({{@$invoiceInfo->recordDetail->section->section_name}}) @if(shiftEnable() &&
                                  @$invoiceInfo->recordDetail->shift) {{{@$invoiceInfo->recordDetail->shift->name}}}
                                  @endif</span> </p>
                              <p><span>@lang('student.roll_no')</span> <span>:
                                  {{$invoiceInfo->recordDetail->roll_no}}</span> </p>
                              <p><span>@lang('student.admission_no')</span> <span>:
                                  {{$invoiceInfo->studentInfo->admission_no}}</span> </p>
                            </div>
                          </td>
                        </tr>
                      </tbody>
                          ...
                          <td>
                            ...
                            <div class="addressright_text">
                              @php
                                ...
                                $stored = $invoiceInfo->payment_status ?? null; // full, partial, not
      ...
      <table class="table border_table mb_30 description_table">
        ...
        <tbody>
          ...
          $total = ($invoiceDetail->amount+ $invoiceDetail->fine) - ($invoiceDetail->paid_amount +
          $invoiceDetail->weaver);
          $balance += $total;
          @endphp
          <tr>
            <td>{{$key+1}}</td>
        ...
        <tfoot>
          <tr>
            <td colspan="5"></td>
            <td colspan="2">
              <p class="total_count"><span>@lang('fees::feesModule.total_amount')</span>
                <span>{{currency_format($amount)}}</span></p>
            </td>
          </tr>
          <tr>
            <td colspan="5"></td>
            <td colspan="2">
              <p class="total_count"><span>@lang('fees::feesModule.total_waiver')</span>
                <span>{{currency_format($weaver)}}</span></p>
            </td>
          </tr>
          <tr>
            <td colspan="5"></td>
            <td colspan="2">
              <p class="total_count"><span>@lang('fees::feesModule.total_fine')</span>
                <span>{{currency_format($fine)}}</span></p>
            </td>
          </tr>
          <tr>
            <td colspan="5"></td>
            <td colspan="2">
              <p class="total_count"><span>@lang('fees::feesModule.total_service_charge')</span>
                <span>{{currency_format($service_charge)}}</span></p>
            </td>
          </tr>
          <tr>
            <td colspan="5"></td>
            <td colspan="2">
              <p class="total_count"><span>@lang('fees::feesModule.total_paid')</span>
                <span>{{currency_format($paid_amount)}}</span></p>
            </td>
          </tr>
          <tr>
            <td colspan="5"></td>
            <td colspan="2">
              <p class="total_count"><span>@lang('accounts.grand_total')</span>
                <span>{{currency_format($grand_total + $service_charge)}}</span></p>
            </td>
          </tr>
          <tr>
            <td colspan="5"></td>
