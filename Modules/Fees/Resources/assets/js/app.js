...
$(document).ready(function () {
  const $studentTypeFilter = $("#studentTypeFilter");
  const hasStudentTypeFilter = $studentTypeFilter.length > 0;

  function setStudentTypeFilterState(classId) {
    if (!hasStudentTypeFilter) {
      return;
    }
    if (classId) {
      $studentTypeFilter.prop("disabled", false);
    } else {
      $studentTypeFilter.val("");
      $studentTypeFilter.prop("disabled", true);
    }
    $studentTypeFilter.niceSelect("update");
  }
  ...
  function fetchStudentsByFilter(classId, studentType) {
    let url = $("#url").val();
    let i = 0;
    if (!classId) {
      setStudentTypeFilterState("");
              ...
              if (allStudent.student_detail.full_name) {
                $("#selectStudent").append(
                  $("<option>", {
                    value: allStudent.id,
                    text:
                      allStudent.student_detail.full_name +
                      " " +
                      "(" +
                      section +
                      " - " +
  ...
  function getSelectedClassId() {
    return $("#select_class").val() || $("#selectClass").val();
  }
  ...
  $("#selectClass, #select_class").on("change", function () {
    ...
    if (hasStudentTypeFilter) {
      $studentTypeFilter.val("");
      setStudentTypeFilterState(classId);
    }
    fetchStudentsByFilter(
      classId,
      hasStudentTypeFilter ? $studentTypeFilter.val() : ""
    );
  });

  if (hasStudentTypeFilter) {
    $studentTypeFilter.on("change", function () {
      let classId = getSelectedClassId();
      if (!classId) {
        return;
      }
      fetchStudentsByFilter(classId, $(this).val());
    });

    if (getSelectedClassId()) {
      setStudentTypeFilterState(getSelectedClassId());
    }
  }
      ...
      if (a.length === 0) {
        ...
        $(".fees").each(function () {
          if ($(this).val() === type) {
            found = false;
            toastr.warning(
              "This Fees Group / Type id Already Exists",
              "Warning",
              { timeOut: 5000 }
            );
          }
  ...
  $("#invSetting").on("click", function (event) {
    ...
    if (invoicePositionValues == "" || uniqIdStart == "" || weaver == "") {
      toastr.warning("Required Field Can Not be Blank", "Warning", {
        timeOut: 5000,
      });
    } else {
      $.ajax({
...
function weaverCalculation(el) {
  ...
  let weaver = parseFloat($(el).closest("tr").find($(".weaver")).val()).toFixed(
    2
  );
    ...
    if (weaver > 0) {
      if (weaverType == "amount") {
        if (amount >= weaver) {
          ...
          toastr.warning("Weaver is less than Amount", "Warning", {
            timeOut: 5000,
          });
        ...
        if (weaver < 101) {
          ...
          toastr.warning("Weaver is not grater than 100", "Warning", {
            timeOut: 5000,
          });
          subTotalAmount(el, amount);
          $(el).closest("tr").find($(".weaver")).val(0);
...
function paidAmountCalculation(el) {
  if (paymentType == "partial" || paymentType == "full") {
    let subTotal = parseFloat(
      $(el).closest("tr").find($(".inputSubTotal")).val()
    );
    if (subTotal == "") {
      ...
      let paidAmountValue = parseFloat(
        $(el).closest("tr").find($(".paidAmount")).val()
      );
      let subTotal = parseFloat(
        $(el).closest("tr").find($(".inputSubTotal")).val()
      );
      if (paidAmountValue > 0) {
        if (subTotal < paidAmountValue) {
          toastr.warning(
            "Paid Amount is not grater than Stub Total ",
            "Warning",
            { timeOut: 5000 }
          );
          $(el).closest("tr").find($(".paidAmount")).val(0);
...
function validWeaver(el) {
  ...
  let previousWeaver = parseFloat(
    $(el).closest("tr").find($(".previousWeaver")).val()
  );
  if (previousWeaver > weaver) {
    parseFloat(
      $(el).closest("tr").find($(".addFeesWeaver")).val(previousWeaver)
    );
  }
}
function addFeesCalculation(el) {
  let weaverType = $(".weaverType").val();
  let showTotalValue = parseFloat(
    $(el).closest("tr").find($(".showTotalValue")).val()
  );
  let dueAmount = parseFloat($(el).closest("tr").find($(".dueAmount")).val());
  let addFeesPaidAmount = parseFloat(
    $(el).closest("tr").find($(".addFeesPaidAmount")).val()
  );
...
function addFeesTotal(el) {
  ...
  let fine = parseFloat($(el).closest("tr").find($(".addFeesFine")).val());
  let subTotal = parseFloat(
    $(el).closest("tr").find($(".addFeesSubTotal")).val()
  );
    ...
    success: function (data) {
      ...
      $.each(data, function (i, item) {
        if (item.length) {
          ...
          $("#feesSectionDiv ul").find("li").not(":first").remove();
          ...
          $("#feesSectionDiv .current").html("SELECT SECTION *");
          $("#feesSection").find("option").not(":first").remove();
          $("#feesSectionDiv ul").find("li").not(":first").remove();
        }
      });
