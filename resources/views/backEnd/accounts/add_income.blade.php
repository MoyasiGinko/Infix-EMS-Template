            ...
            <div class="white-box">
              ...
              let html = '<table id="' + id +
                ...
                <tbody>';
                  rows.forEach(r => {
                  html += '<tr>
                    <td>' + escapeIncomeHtml(r.date) + '</td>
                    <td>' + escapeIncomeHtml(r.name) + '</td>
                    <td>' +
                      escapeIncomeHtml(r.identifier) + '</td>
                    <td>' + escapeIncomeHtml(r.payment_method) + '</td>
                    <td>' +
                      escapeIncomeHtml(r.details) + '</td>
                    <td class="text-right">' +
                      escapeIncomeHtml(r.amount_display || formatIncomeAmount(r.amount)) + '</td>
                  </tr>';
                  });
                  html +=
                  '</tbody>
                <tfoot>
                  <tr style="font-weight:bold;background:#f5f5f5;">
                    ...
                    <td class="text-right">' +
                      escapeIncomeHtml(formatIncomeAmount(grand)) + '</td>
                  </tr>
                </tfoot>
              </table>';
              $c.append(html);
              ...
              for (let i = 0; i < bytes.length; i++) { bin +=String.fromCharCode(bytes[i]); } pdfMake.addFileToVFS(file,
                btoa(bin)); loaded=true; break; } } catch (_) {} } if (!loaded) return false; }
                pdfMake.fonts=pdfMake.fonts || {}; pdfMake.fonts.BanglaFont={ normal: 'NotoSansBengali-Regular.ttf' ,
                bold: 'NotoSansBengali-Bold.ttf' , italics: 'NotoSansBengali-Regular.ttf' ,
                bolditalics: 'NotoSansBengali-Bold.ttf' }; window._banglaFontReady=true; return true; } catch (e) {
                return false; } } function escapeCsv(value) { return (value ?? '' ).toString().replace(/"/g, '""' ); }
                function manualCSV(rows) { let csv='"Date","Name","Identifier","Payment Method","Details","Amount"\n' ;
                rows.forEach(r=> {
                ...
                <body>' +
                  document.getElementById(id).outerHTML + '</body>
                              ...
                              <tbody>
                                ...
                                <tr class="invoice-group-row" data-export='@json($invoiceExportPayload)'>
                                  <td class="text-center">
                                    <span
                                      class="badge badge-primary badge-pill">&sum;{{ $group['entries'] ?? 0 }}</span>
                                  </td>
                                  <td class="font-weight-500">
                                    ...
                                    <div class="text-muted small">@lang('fees.invoice_number'):
                                      <span
                                        class="font-weight-600">{{ $meta['invoice_number'] ?? __('common.na') }}</span>
                                    </div>
                                  </td>
                                  <td>
                                  ...
                                  <td class="text-right font-weight-600"
                                    data-amount="{{ $group['total_amount'] ?? 0 }}">
                              ...
                              <tbody>
                                ...
                                <tr data-export='@json($manualExportPayload)'>
                                  ...
                                  <td>
                                    ...
                                    <span
                                      class="badge badge-outline-info">{{ $row->invoiceInfo->invoice_number }}</span>
                                    @else
                                    <span class="text-muted">@lang('common.na')</span>
                                    @endif
                                  </td>
