...
if(!function_exists('getMenus'))
{
    ...
    {
        try {
            if (!\Illuminate\Support\Facades\Schema::hasTable('sm_menus')) {
                \Illuminate\Support\Facades\Schema::create('sm_menus', function (\Illuminate\Database\Schema\Blueprint $table) {
                    $table->id();
            ...
            if (!\Illuminate\Support\Facades\Schema::hasTable('default_menus')) {
                \Illuminate\Support\Facades\Schema::create('default_menus', function (\Illuminate\Database\Schema\Blueprint $table) {
                    $table->id();
                    $table->string('name')->nullable();
                    $table->string('module')->nullable();
                    $table->string('route')->nullable();
            ...
            if (\Illuminate\Support\Facades\Schema::hasTable('sm_menus')) {
                $count = \Illuminate\Support\Facades\DB::table('sm_menus')->count();
                if ($count == 0) {
                    $menuFile = base_path('Modules/MenuManage/Resources/var/menus.sql');
                    if (file_exists($menuFile)) {
                        \Illuminate\Support\Facades\DB::statement('SET FOREIGN_KEY_CHECKS=0;');
                        \Illuminate\Support\Facades\DB::unprepared(file_get_contents($menuFile));
            ...
            if (\Illuminate\Support\Facades\Schema::hasTable('default_menus')) {
                $countDefault = \Illuminate\Support\Facades\DB::table('default_menus')->count();
                if ($countDefault == 0) {
                    $defaultMenuFile = base_path('Modules/MenuManage/Resources/var/default_menus.sql');
                    if (file_exists($defaultMenuFile)) {
                        \Illuminate\Support\Facades\DB::statement('SET FOREIGN_KEY_CHECKS=0;');
                        \Illuminate\Support\Facades\DB::unprepared(file_get_contents($defaultMenuFile));
                        \Illuminate\Support\Facades\DB::statement('SET FOREIGN_KEY_CHECKS=1;');
                    }
                }
            }
        } catch (\Throwable $e) {
            \Illuminate\Support\Facades\Log::warning('Error auto-seeding menus: ' . $e->getMessage());
        }
